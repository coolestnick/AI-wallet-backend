...
class PortfolioManagementAgent(Agent):
    """
    Portfolio Management Agent provides guidance on cryptocurrency portfolio optimization,
    risk management, and investment strategies.
    """
    
    def __init__(self, model_id=None, **kwargs):
        config = get_model_config("portfolio_analysis")
        model_id = model_id or config["model_id"]
        super().__init__(
            name="Portfolio Management Agent",
            agent_id="portfolio_management",
            description="Expert guidance on cryptocurrency portfolio optimization, risk management, and investment strategies",
        )
        api_key = os.getenv("GEMINI_API_KEY")
        if not api_key:
            logging.error("GEMINI_API_KEY environment variable is not set. Please add it to your .env file.")
            self.logger = logging.getLogger("portfolio_management")
    ...
    def _format_messages_for_gemini(self, messages):
        ...
        try:
            ...
            # Final fallback
            return "Hello, how can I help you with your cryptocurrency portfolio today?"
...
def get_portfolio_management_agent(
    model_id: str = "gemini-2.0-flash-lite",
    user_id: Optional[str] = None,
    session_id: Optional[str] = None,
    debug_mode: bool = True,
) -> Agent:
    """Get a Portfolio Management agent"""
